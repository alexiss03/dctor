...
* $c->pg_connect.
*/
$c->pg_connect[] = "dbname={$_ENV['PGSQL_DB']} user={$_ENV['PGSQL_USER']} port=5432 host=rdb password={$_ENV['PGSQL_PASSWORD']}";
$c->db_schema = "caldav";
...
function OAuth2Check($username, $password) {
    error_log($_SERVER['AUTH_TYPE']);
    $headers = getallheaders();
    if(
       (isset($headers['OIDC_CLAIM_token_type']) && $headers['OIDC_CLAIM_token_type'] == 'Bearer')
       && (isset($headers['OIDC_CLAIM_active']) && $headers['OIDC_CLAIM_active'] == 1)) {
              // foreach ($_SERVER as $header => $value) {
              //        error_log($header .' '. $value);
              // }
       return true;
    } else 
        return LDAP_check($username, $password);
}
$c->authenticate_hook['call'] = 'OAuth2Check';

$c->authenticate_hook['server_auth_type'] = array('Basic', 'LDAP');
...
$c->authenticate_hook['config'] = array(
    'host' => 'iam', //host name of your LDAP Server
    'port' => '389',              //port
...
// DN and password if rquired to bind to this server for initial queries
    ...
    'baseDNUsers'  => 'ou=dctor',              // where to look at valid user
    'filterUsers'  => '(cn=*)',  // filter which must validate a user according to RFC4515, i.e. surrounded by brackets
