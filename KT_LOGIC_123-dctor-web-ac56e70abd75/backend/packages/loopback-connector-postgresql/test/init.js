...
let config = require('rc')('loopback', {test: {postgresql: {}}}).test
  .postgresql;

process.env.PGHOST =
  process.env.POSTGRESQL_HOST || process.env.PGHOST || 'localhost';
process.env.PGPORT = process.env.POSTGRESQL_PORT || process.env.PGPORT || 5432;
process.env.PGUSER =
...
config = {
  ...
  database:
    process.env.POSTGRESQL_DATABASE || process.env.PGDATABASE || 'emptytest',
