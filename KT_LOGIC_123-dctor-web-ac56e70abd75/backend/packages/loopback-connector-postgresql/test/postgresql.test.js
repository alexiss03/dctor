...
describe('postgresql connector', function () {
  ...
  it('should preserve property `count` after query execution', function (done) {
    Post.create({title: 'T10', content: 'C10'}, function (err, p) {
      if (err) return done(err);
      post = p;
      const query =
        "UPDATE PostWithBoolean SET title ='T20' WHERE id=" + post.id;
  ...
  it('should support `rows` if RETURNING used after UPDATE', function (done) {
    Post.create(
      ...
      function (err, p) {
        ...
        post = p;
        const query =
          "UPDATE PostWithBoolean SET title ='something else' WHERE id=" +
          post.id +
          ' RETURNING id';
        db.connector.execute(query, function (err, results) {
          results.should.have.property('count', 1);
  ...
  it('should support full text search for text type fields using simple string query', async () => {
    ...
    found
      .map(p => p.title)
      .should.deepEqual(['Loopback joined the OpenJS Foundation']);
  });
  ...
  it('should return the model instance for upsert', function (done) {
    Post.upsert(
      {idCol: post.id, title: 'T2_new', content: 'C2_new', approved: true},
      function (err, p) {
  ...
  it('should escape number values to defect SQL injection in find (where)', function (done) {
  ...
  it('should escape number values to defect SQL injection in find (limit)', function (done) {
  ...
  context('GeoPoint types', function () {
    ...
    it('should support complicated where filter', function (done) {
      Post.find(
        {where: {and: [{loc: loc}, {title: {like: '%t%'}}]}},
        function (err, post) {
          should.not.exist(err);
          should.exist(post);
    ...
    it('should preserve order of and/or in where', async function () {
      ...
      const posts = await Post.find({
        where: {or: [{title: 'T3'}, {approved: false}], content: 'C2'},
      });
  ...
  context('pattern matching operators', function () {
    ...
    it('supports negative case insensitive queries using nilike', function (done) {
  ...
  context('regexp operator', function () {
    ...
    context('with regex strings', function () {
      ...
      context('using flags', function () {
        ...
        it('should print a warning when the multiline flag is set', function (done) {
          Post.find(
            {where: {content: {regexp: '^a/m'}}},
            function (err, posts) {
              console.warn.calledOnce.should.be.ok;
              done();
            },
    ...
    context('with regex objects', function () {
      ...
      context('using flags', function () {
        ...
        it('should print a warning when the multiline flag is set', function (done) {
          Post.find(
            {where: {content: {regexp: new RegExp(/^a/m)}}},
            function (err, posts) {
              console.warn.calledOnce.should.be.ok;
              done();
            },
