...
describe('Order settings', function () {
  ...
  it('builds base query', function (done) {
    ...
    sql.should.be.equal(
      'SELECT "id","title","content" FROM "public"."postwithdefaultidsort" ORDER BY "id"',
    );
    done();
  });
  ...
  it('builds non-ordering base query', function (done) {
    ...
    sql.should.be.equal(
      'SELECT "id","title","content" FROM "public"."postwithdisableddefaultidsort"',
    );
    done();
  });
  ...
  it('builds ordering query based on numericIdOnly setting', function (done) {
    ...
    sql.should.be.equal(
      'SELECT "id","title","content" FROM "public"."postwithnumericdefaultidsort" ORDER BY "id"',
    );
    done();
  });

  it('builds non-ordering query based on numericIdOnly setting', function (done) {
    const res = db.connector.buildSelect(
      'PostWithNumericStringDefaultIdSort',
      {},
    );
    const sql = res.sql;
    ...
    sql.should.be.equal(
      'SELECT "id","title","content" FROM "public"."postwithnumericstringdefaultidsort"',
    );
    done();
  });

  it('should return results ordered by id', function (done) {
    PostWithDefaultIdSort.create(
      {id: 5, title: 'c', content: 'CCC'},
      function (err, post) {
        PostWithDefaultIdSort.create(
          {id: 4, title: 'd', content: 'DDD'},
          function (err, post) {
  ...
  it('should return unordered results', function (done) {
    PostWithDisabledDefaultIdSort.create(
      {id: 2, title: 'c', content: 'CCC'},
      function (err, post) {
        PostWithDisabledDefaultIdSort.create(
          {id: 1, title: 'd', content: 'DDD'},
          function (err, post) {
            PostWithDisabledDefaultIdSort.find({}, function (err, posts) {
              ...
              PostWithDisabledDefaultIdSort.find(
                {limit: 1, offset: 0},
                function (err, posts) {
                  should.not.exist(err);
                  posts.length.should.be.equal(1);
  ...
  it('Should return ordered results by numeric id by default', function (done) {
    PostWithNumericDefaultIdSort.create(
      {id: 12, title: 'c', content: 'CCC'},
      function (err, post) {
        PostWithNumericDefaultIdSort.create(
          {id: 11, title: 'd', content: 'DDD'},
          function (err, post) {
            PostWithNumericDefaultIdSort.find({}, function (err, posts) {
              ...
              PostWithNumericDefaultIdSort.find(
                {limit: 1, offset: 0},
                function (err, posts) {
                  should.not.exist(err);
                  posts.length.should.be.equal(1);
  ...
  it('should return unordered results because of string type id', function (done) {
    PostWithNumericStringDefaultIdSort.create(
      {id: 'b', title: 'c', content: 'CCC'},
      function (err, post) {
        PostWithNumericStringDefaultIdSort.create(
          {id: 'a', title: 'd', content: 'DDD'},
          function (err, post) {
            PostWithNumericStringDefaultIdSort.find({}, function (err, posts) {
              ...
              PostWithNumericStringDefaultIdSort.find(
                {limit: 1, offset: 0},
                function (err, posts) {
                  should.not.exist(err);
                  posts.length.should.be.equal(1);
