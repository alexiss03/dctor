...
describe('transactions', function () {
    ...
    return function (done) {
      Transaction.begin(
        ...
        function (err, tx) {
          ...
          currentTx = tx;
          Post.create(post, {transaction: tx}, function (err, p) {
            if (err) {
              done(err);
            } else {
              done();
  ...
  function expectToFindPosts(where, count, inTx) {
    return function (done) {
      ...
      Post.find({where: where}, options, function (err, posts) {
        if (err) return done(err);
        posts.length.should.be.eql(count);
        // Make sure both find() and count() behave the same way
        Post.count(where, options, function (err, result) {
          if (err) return done(err);
  ...
  describe('commit', function () {
    ...
    it(
      'should see the uncommitted insert from the same transaction',
