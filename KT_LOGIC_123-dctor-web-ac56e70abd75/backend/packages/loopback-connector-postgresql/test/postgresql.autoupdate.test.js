...
describe('autoupdate', function () {
    ...
    ds.automigrate(() => {
      ds.discoverModelProperties('customer_test', (err, props) => {
        ...
        props = props.sort((a, b) => {
          ...
          if (
            _.indexOf(schemaKeys, a.columnName) >
            _.indexOf(schemaKeys, b.columnName)
          )
            return 1;
          if (
            _.indexOf(schemaKeys, a.columnName) <
        ...
        ds.connector.discoverModelIndexes('CustomerTest', (err, indexes) => {
          ...
          ds.autoupdate((err, result) => {
            ds.discoverModelProperties('customer_test', (err, props) => {
              ...
              ds.connector.discoverModelIndexes(
                'CustomerTest',
                (err, indexes) => {
                  assert.deepEqual(indexes, {
                    customer_test_pkey: {
  ...
  it('should produce valid sql for setting column nullability', function (done) {
    ...
    ds.automigrate(function () {
      ...
      ds.connector.getTableStatus(schema_v2.name, function (err, actualFields) {
        const sql = ds.connector.getPropertiesToModify(
          schema_v2.name,
          actualFields,
        )[0];
        assert.equal(
          ...
          'Check that the SQL is correctly spaced.',
        );
        done();
      });
    });
  ...
  describe('foreign key constraint', function () {
    it('should create, update, and delete foreign keys', function (done) {
      ...
      ds.createModel(
        customer2_schema.name,
        customer2_schema.properties,
        customer2_schema.options,
      );
      ...
      ds.autoupdate(function (err) {
        ...
        ds.createModel(
          ...
          product_schema.options,
        );
        // do initial update/creation of table with fk
        // model OrderTest has a fk refers to model CustomerTest2
        ds.createModel(
          orderTest_schema_v1.name,
          orderTest_schema_v1.properties,
          orderTest_schema_v1.options,
        );
        ds.autoupdate(function (err) {
          if (err) {
          ...
          ds.discoverModelProperties('order_test', function (err, props) {
            ...
            ds.connector.discoverForeignKeys(
              'order_test',
              {},
              function (err, foreignKeys) {
                ...
                ds.createModel(
                  orderTest_schema_v2.name,
                  orderTest_schema_v2.properties,
                ...
                ds.autoupdate(function (err) {
                  ...
                  ds.connector.discoverForeignKeys(
                    ...
                    function (err, foreignKeys) {
                      ...
                      ds.createModel(
                        orderTest_schema_v3.name,
                        orderTest_schema_v3.properties,
                        orderTest_schema_v3.options,
                      );
                      ds.autoupdate(function (err) {
                        if (err) {
                          err.message +=
                            ' (while updating OrderTest schema v3)';
                          return done(err);
                        }
                        ds.discoverModelProperties(
                          'order_test',
