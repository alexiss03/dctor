...
describe('Discover model properties', function () {
  ...
  it(
    'coerces `real` and `data precision` types to float on model prop ' +
      'discovery',
    function (done) {
  ...
  it('should return an LDL schema for inventory', function (done) {
    db.discoverSchema(
      ...
      function (err, schema) {
        ...
        assert(
          schema.properties.productId.postgresql.columnName === 'product_id',
        );
        assert(schema.properties.locationId);
        assert(schema.properties.locationId.type === 'String');
        assert(
          schema.properties.locationId.postgresql.columnName === 'location_id',
        );
    ...
    db.discoverSchema(
      ...
      function (err, schema) {
        if (err) {
          ...
          assert(
            schema.properties.lastlogin.postgresql.dataType ===
              'timestamp with time zone',
          );
          assert(schema.properties.created);
          assert(schema.properties.created.type === 'Date');
          assert(
            schema.properties.created.postgresql.dataType ===
