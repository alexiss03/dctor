...
function mixinMigration(PostgreSQL) {
   ...
   */
  PostgreSQL.prototype.alterTable = function (
    model,
    actualFields,
    actualIndexes,
    cb,
    ...
    const indexNames = m.settings.indexes
    ...
    self.discoverForeignKeys(self.table(model), {}, function (err, actualFks) {
      if (err) {
        debug(
          'Failed to discover "%s" foreign keys %s',
          self.table(model),
          err,
        );
        cb(err);
      ...
      async.series(
        [
          ...
          function (cb) {
            ...
            const fkSQL = self.getForeignKeySQL(
              model,
  ...
  PostgreSQL.prototype.getPropertiesToModify = function (model, actualFields) {
    ...
    propNames.forEach(function (propName) {
      ...
      if (found && self.propertyHasNotBeenDeleted(model, propName)) {
        if (datatypeChanged(propName, found)) {
          sql.push(
            'ALTER COLUMN ' + self.modifyDatatypeInActual(model, propName),
          );
        }
        if (nullabilityChanged(propName, found)) {
          sql.push(
            'ALTER COLUMN ' + self.modifyNullabilityInActual(model, propName),
          );
        }
      }
    });
    ...
    function datatypeChanged(propName, oldSettings) {
      ...
      return (
        oldSettings.type.toUpperCase() !== self.columnDataType(model, propName)
      );
    }
  ...
  PostgreSQL.prototype.buildColumnDefinitions =
    PostgreSQL.prototype.propertiesSQL = function (model) {
      ...
      Object.keys(this.getModelDefinition(model).properties).forEach(function (
        prop,
      ) {
  ...
  PostgreSQL.prototype.buildColumnDefinition = function (model, propName) {
    ...
    if (prop.generated) {
      if (result === 'INTEGER') {
        ...
        if (
          postgSettings &&
          postgSettings.defaultFn &&
          postgSettings.extension
        ) {
        ...
        console.log(
          chalk.red('>>> WARNING: ') +
            `auto-generation is not supported for type "${chalk.yellow(
              prop.type,
            )}". \
  ...
  PostgreSQL.prototype.createTable = function (model, cb) {
    ...
    Object.keys(this.getModelDefinition(model).properties).forEach(function (
      ...
      if (
        postgSettings &&
        postgSettings.dataType &&
        postgSettings.dataType === 'UUID' &&
        postgSettings.defaultFn &&
        postgSettings.extension
      ) {
        createExtensions +=
          'CREATE EXTENSION IF NOT EXISTS "' + postgSettings.extension + '";';
      }
    });
    // default extension
    if (!createExtensions) {
    ...
    self.execute(
      createExtensions + 'CREATE SCHEMA ' + self.escapeName(self.schema(model)),
      function (err) {
  ...
  PostgreSQL.prototype.buildIndex = function (model, property) {
    ...
    if (kind && type) {
      ...
      typeof i === 'object' &&
        ...
        (kind = 'UNIQUE');
      return (
        kind + ' INDEX ' + columnName + ' ' + type + ' (' + columnName + ') '
      );
    }
  };
  ...
  PostgreSQL.prototype.buildIndexes = function (model) {
    ...
    for (const index in indexes) {
      ...
      if (kind && type) {
        indexClauses.push(
          kind + ' INDEX ' + indexName + ' (' + columns + ') ' + type,
        );
      } else {
        indexClauses.push(
          kind + ' INDEX ' + type + ' ' + indexName + ' (' + columns + ')',
        );
      }
    }
  ...
  PostgreSQL.prototype.columnDataType = function (model, property) {
    ...
    const colLength =
      (columnMetadata && columnMetadata.dataLength) ||
      prop.length ||
      prop.limit;
  ...
  PostgreSQL.prototype.buildColumnType = function buildColumnType(
    propertyDefinition,
  ) {
    let returnType = '';
    switch (propertyDefinition.type.name) {
      ...
      case 'Point':
        ...
        break;
      case 'Boolean':
        returnType += 'BOOLEAN'; // PostgreSQL doesn't have built-in boolean
        break;
    }
    let expr = '';
    if (propertyDefinition.postgresql.generatedExpression) {
      if (propertyDefinition.postgresql.generatedDataType) {
        returnType = propertyDefinition.postgresql.generatedDataType;
      }
      expr = ` GENERATED ALWAYS AS ${propertyDefinition.postgresql.generatedExpression}`;
    }
    returnType += expr;
    return returnType;
  };
    ...
    if (fks && fks.length) {
      ...
      fks.forEach(function (fk) {
        ...
        if (newFk) {
          ...
          const fkEntityName =
            typeof newFk.entity === 'object' ? newFk.entity.name : newFk.entity;
          const fkRefTable = self.table(fkEntityName);
          needsToDrop =
            !isCaseInsensitiveEqual(fkCol, fk.fkColumnName) ||
  ...
  PostgreSQL.prototype.getForeignKeySQL = function getForeignKeySQL(
    model,
    actualFks,
    existingFks,
  ) {
    ...
    if (actualFks) {
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        if (
          existingFks.filter(function (fk) {
  ...
  PostgreSQL.prototype.buildForeignKeyDefinition =
    function buildForeignKeyDefinition(model, keyName) {
      const definition = this.getModelDefinition(model);
      ...
      if (fk) {
        ...
        const fkEntityName =
          typeof fk.entity === 'object' ? fk.entity.name : fk.entity;

        // verify that the other model in the same DB
        if (this._models[fkEntityName]) {
          return (
            'CONSTRAINT ' +
            this.escapeName(fk.name) +
            ' ' +
            'FOREIGN KEY (' +
            this.escapeName(fk.foreignKey) +
            ') ' +
            'REFERENCES ' +
            this.tableEscaped(fkEntityName) +
            '(' +
            fk.entityKey +
            ') ' +
            'ON DELETE ' +
            parseAction(fk.onDelete) +
            ' ' +
            'ON UPDATE ' +
            parseAction(fk.onUpdate)
          );
        }
      }
      return '';
  ...
  PostgreSQL.prototype.addIndexes = function (model, actualIndexes, cb) {
    ...
    aiNames.forEach(function (indexName) {
      ...
      if (i.primary || (m.properties[indexName] && self.id(model, indexName)))
        return;
        ...
        if (propName) {
          ...
          if (m.properties[propName] && (si = m.properties[propName].index)) {
            if (
              ...
              sql.push(
                'DROP INDEX ' + schema + '.' + self.escapeName(indexName),
              );
              delete ai[indexName];
            }
          }
          ...
          if (identical && siKeys.length > 1) {
            if (siKeys.length !== i.keys.length) {
              ...
              siKeys.forEach(function (propName, iter) {
                identical =
                  identical && self.column(model, propName) === i.keys[iter];
              });
            }
          }
    ...
    indexNames.forEach(function (indexName) {
      ...
      if (!found) {
        ...
        const columns = i.keys
          .map(function (key) {
            return (
              self.escapeName(self.column(model, key[0])) +
              (key[1] ? ' ' + key[1] : '')
            );
          })
          .join(', ');
  ...
  function normalizeIndexKeyDefinition(keys) {
    ...
    if (typeof keys === 'string') {
      ...
      throw Error(
        g.f('{{Index keys}} definition appears to be invalid: %s', keys),
      );
    }
