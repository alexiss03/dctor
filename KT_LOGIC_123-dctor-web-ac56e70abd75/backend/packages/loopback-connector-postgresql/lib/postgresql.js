...
PostgreSQL.prototype.buildExpression = function (
  ...
  switch (operator) {
    ...
    case 'match':
      return new ParameterizedSQL(
        `to_tsvector(${columnName}) @@ to_tsquery(?)`,
        [operatorValue],
      );
    case 'jpquery':
      const JSON_PATH_OPS = [
      ...
      const JSON_CAST = [
        'jsonb', 'text', 'integer', 'jsonpath',
      ];
      if(JSON_PATH_OPS.includes(operatorValue.op)
          && JSON_CAST.includes(operatorValue.cast))
        return new ParameterizedSQL(
          `${
            operatorValue.check_length === 'true'?'jsonb_array_length(':''
          }jsonb_path_query_array(${columnName}::jsonb, ?::jsonpath, ?::jsonb)${
            operatorValue.check_length === 'true'?')':''
          } ${operatorValue.op || '@>'} ?::${operatorValue.cast || 'jsonb'}`,
          [
            operatorValue.path || '$',
            JSON.stringify(operatorValue.vars || {}),
            operatorValue.value || '',
          ],
        );
    default:
